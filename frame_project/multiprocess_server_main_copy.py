...
class ServerManager:
    ...
    Attributes(公共属性):
        client_port(str): 公司外部客户端对应服务器的端口
        internal_use_port(str): 公司内部客户端对应服务器的端口
        route_dict(dict): 接口路由和 API 函数之间关系的字典
        api_func_dict(dict): 动态引入生成的 API 函数字典
    """
