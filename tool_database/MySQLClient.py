...
from typing import Any, List, Dict, Optional, Tuple  # 引入类型提示
...
class MySQLClient:
    ...
    def transaction(self, queries: List[str], params_list: List[List[Any]]) -> bool:
        ...
        if len(queries) != len(params_list):
            raise ValueError("The number of queries must match the number of parameter lists.")
        ...
        try:
            ...
            print(f"Transaction failed with error: {e}")  # 打印错误信息
    ...
    def update(self, table: str, data: Dict[str, Any], where: Dict[str, Any]) -> int:
        ...
        return self.execute(query, list(data.values()) + list(where.values()))
            ...
            with self.conn.cursor() as cursor:
                for op in operations:
                    ...
                    if action == "insert":
                        ...
                        affected_rows = self.delete(table, where)
                        if affected_rows == 0:  # 如果没有删除任何行，则回滚事务
                            raise ValueError(f"Delete failed: No rows affected for {where}")
                    elif action == "select":
                        print(self.select(table, where))
                    else:
...
if __name__ == "__main__":
    ...
    #         db.conn.rollback()  # 手动回滚，不提交
    # except Exception as e:
    #     print("Manual transaction failed:", e)
    #
    # print("All Fetch After Manual Rollback:", db.fetch_all("SELECT * FROM users"))  # 确保数据未插入
    #
    # db.close()  # 关闭数据库连接